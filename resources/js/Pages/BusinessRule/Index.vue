...
import { BusinessRulesData } from "@/types/index";
import { ref, watch, PropType } from "vue";
import Flasher from "@/helprs";
import { FlasherResponse } from "@flasher/flasher";
import CreateBusinessRule from "./CreateBusinessRule.vue";
import EditBusinessRule from "./EditBusinessRule.vue";
import DeleteBusinessRule from "./DeleteBusinessRule.vue";

const props = defineProps({
    regrasDeBusinessRule: {
        type: Object as PropType<BusinessRulesData>,
        required: true,
    },
    search: String,
...
const links = ref(props.regrasDeBusinessRule.links);

const editingBusinessRuleTrigger = ref(false);
const editingBusinessRule = ref<App.Data.BusinessRuleData | null>(null);

const deletingBusinessRuleTrigger = ref(false);
const deletingBusinessRule = ref<App.Data.BusinessRuleData | null>(null);
...
watch(
    () => props.regrasDeBusinessRule.links,
    (value) => {
        links.value = value;
    }
);
...
function openEditBusinessRuleModal(business: App.Data.BusinessRuleData) {
    editingBusinessRule.value = business;
    editingBusinessRuleTrigger.value = true;
}

function closeEditBusinessRuleModal() {
    editingBusinessRule.value = null;
    editingBusinessRuleTrigger.value = false;
}

function openDeleteBusinessRuleModal(business: App.Data.BusinessRuleData) {
    deletingBusinessRule.value = business;
    deletingBusinessRuleTrigger.value = true;
}

function closeDeleteBusinessRuleModal() {
    deletingBusinessRule.value = null;
    deletingBusinessRuleTrigger.value = false;
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template v-slot:content>
                        ...
                        >
                            ...
                            <tbody>
                                <tr
                                    ...
                                    v-for="business in regrasDeBusinessRule.data"
                                    :key="(business.id as number)"
                                >
                                    <th
                                        scope="row"
                                    ...
                                    <td class="px-4 py-3 w-32">
                                        <button
                                            type="button"
                                            @click="
                                                openEditBusinessRuleModal(business)
                                            "
                                    ...
                                    <td class="px-4 py-3 justify-end w-32">
                                        <button
                                            type="button"
                                            @click="
                                                openDeleteBusinessRuleModal(business)
                                            "
                    ...
                    >
                        ...
                        >
                            ...
                            <span
                                ...
                                >{{
                                    `${regrasDeBusinessRule.meta.from ?? 0}-${
                                        regrasDeBusinessRule.meta.to ?? 0
                                    }`
                                }}</span
                            >
                            of
                            <span
                                class="font-semibold text-gray-900 dark:text-white"
                            >
                                {{ regrasDeBusinessRule.meta.total }}</span
                            >
                        </span>
            ...
            <EditBusinessRule
                v-if="editingBusinessRule"
                :business="editingBusinessRule"
                :openModal="editingBusinessRuleTrigger"
                :close="closeEditBusinessRuleModal"
            />
            <DeleteBusinessRule
                v-if="deletingBusinessRule"
                :business="deletingBusinessRule"
                :openModal="deletingBusinessRuleTrigger"
                :close="closeDeleteBusinessRuleModal"
            />
        </template>
    </AuthenticatedLayout>
</template>
