...
const closeCreateConditionModal = () => {
...
const createCondition = () => {
    form.post(route("condition.store"), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
...
<template>
    ...
    <Modal :show="addCondition" @close="closeCreateConditionModal">
        <div class="relative bg-white rounded shadow dark:bg-gray-700">
            <button
                type="button"
                class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                @click="closeCreateConditionModal"
            >
                <svg
                    aria-hidden="true"
                    class="w-5 h-5"
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" @submit.prevent="createCondition">
                    <div>
                        <label
                            for="name"
