...
import { ref } from "vue";
const notAprrovedPropertys = ref(usePage().props.notAprrovedPropertys);
watch(
    () => usePage().props.notAprrovedPropertys,
    (notAprrovedPropertysNumber) =>
        (notAprrovedPropertys.value = notAprrovedPropertysNumber)
);
...
<template>
    <Link
        v-if="authorized()"
        :href="route('property.not.approved.all')"
        v-tooltip.value="tooltip('Imóveis não aprovados')"
    >
        <li
            class="px-3 py-2 rounded mb-0.5 last:mb-0"
            :class="
                route().current('property.not.approved.all*') &&
                route().current() != 'property.all.trash'
                    ? ' bg-slate-950'
                    : 'hover:bg-slate-900'
            "
        >
