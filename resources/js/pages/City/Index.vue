...
const crudManagerRef = ref(crudManager<App.Data.CityData>());
const editManagerRef = ref(crudManager<App.Data.CityData>());
const deleteManagerRef = ref(crudManager<App.Data.CityData>());

const tableData = ref<App.Data.CityData[]>([]);
watch(
    () => props.cities?.data,
    (newData) => {
...
const columnHelper = createColumnHelper<App.Data.CityData>();
const columns = [
    ...
    columnHelper.accessor((row) => row.province?.name, {
        id: 'province_name',
        header: ({ column }) => h(VHeader, { column, title: t('Province name') }),
        cell: (info) => h(VCell, { cell: info, value: info.getValue() }),
    }),
                ...
                <Card class="rounded-[1px] shadow-none">
                    ...
                    <CardFooter class="p-0">
                        <Pagination
                            ...
                            :links="cities?.links"
                            :first_page_url="cities?.first_page_url"
                            :last_page_url="cities?.last_page_url"
                            :next_page_url="cities.next_page_url"
                            :prev_page_url="cities?.prev_page_url"
                            class="p-4"
                        />
                    </CardFooter>
                </Card>
