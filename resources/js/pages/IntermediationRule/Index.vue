...
import Create from './CreateIntermediationRule.vue';
import Edit from './EditIntermediationRule.vue';
import { createColumnHelper } from '@tanstack/vue-table';
import VTable from '@/components/VTable/VTable.vue';
import VHeader from '@/components/VTable/VHeader.vue';
import VCell from '@/components/VTable/VCell.vue';
import Delete from './DeleteIntermediationRule.vue';
...
watch(searchTerm, (value) => {
    router.visit(
        route('intermediation-rule.all', {
            search: value ?? '',
        }),
        {
            only: ['intermediationRules'],
            replace: true,
            preserveState: true,
        },
    );
...
const crudManagerRef = ref(crudManager<App.Data.IntermediationRuleData>());
const editManagerRef = ref(crudManager<App.Data.IntermediationRuleData>());
const deleteManagerRef = ref(crudManager<App.Data.IntermediationRuleData>());

const tableData = ref<App.Data.IntermediationRuleData[]>([]);
watch(
...
const columnHelper = createColumnHelper<App.Data.IntermediationRuleData>();
const columns = [
    ...
    columnHelper.accessor('name', {
        header: ({ column }) => h(VHeader, { column, title: t('Intermediation Rule name') }),
        cell: (info) => h(VCell, { cell: info, value: info.getValue() }),
    }),
    columnHelper.accessor('code', {
        header: ({ column }) => h(VHeader, { column, title: t('Code') }),
        cell: (info) => h(VCell, { cell: info, value: info.getValue(), class: 'font-mono text-sm' }),
    }),
    columnHelper.accessor('percentage', {
        header: ({ column }) => h(VHeader, { column, title: t('Percentage') }),
        cell: (info) => h(VCell, { cell: info, value: `${info.getValue()}%`, class: 'text-right' }),
    }),
...
<template>
    <Head :title="$t('Intermediation Rules')" />
    <AppLayout>
            ...
            <div class="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl">
                <Card class="rounded-[1px] shadow-none">
                    ...
                    <CardFooter class="p-0">
                        <Pagination
                            :from="intermediationRules?.from"
                            :to="intermediationRules.to"
                            :total="intermediationRules.total"
                            :per_page="intermediationRules.per_page"
                            :links="intermediationRules?.links"
                            :first_page_url="intermediationRules?.first_page_url"
                            :last_page_url="intermediationRules?.last_page_url"
                            :next_page_url="intermediationRules.next_page_url"
                            :prev_page_url="intermediationRules?.prev_page_url"
                            class="p-4"
                        />
                    </CardFooter>
                </Card>
