...
import Create from './CreateBusinessRule.vue';
import Edit from './EditBusinessRule.vue';
import { createColumnHelper } from '@tanstack/vue-table';
import VTable from '@/components/VTable/VTable.vue';
import VHeader from '@/components/VTable/VHeader.vue';
import VCell from '@/components/VTable/VCell.vue';
import Delete from './DeleteBusinessRule.vue';
...
const crudManagerRef = ref(crudManager<App.Data.BusinessRuleData>());
const editManagerRef = ref(crudManager<App.Data.BusinessRuleData>());
const deleteManagerRef = ref(crudManager<App.Data.BusinessRuleData>());

const tableData = ref<App.Data.BusinessRuleData[]>([]);
watch(
...
const columnHelper = createColumnHelper<App.Data.BusinessRuleData>();
const columns = [
    ...
    columnHelper.accessor('name', {
        header: ({ column }) => h(VHeader, { column, title: t('Business rule name') }),
        cell: (info) => h(VCell, { cell: info, value: info.getValue() }),
    }),
            ...
            <div class="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl">
                <Card class="rounded-[1px] shadow-none">
                    ...
                    <CardFooter class="p-0">
                        <Pagination
                            :from="businessRules?.from"
                            :to="businessRules.to"
                            :total="businessRules.total"
                            :per_page="businessRules.per_page"
                            :links="businessRules?.links"
                            :first_page_url="businessRules?.first_page_url"
                            :last_page_url="businessRules?.last_page_url"
                            :next_page_url="businessRules.next_page_url"
                            :prev_page_url="businessRules?.prev_page_url"
                            class="p-4"
                        />
                    </CardFooter>
                </Card>
