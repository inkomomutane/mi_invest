...
import Create from './CreatePropertyType.vue';
import Edit from './EditPropertyType.vue';
import { createColumnHelper } from '@tanstack/vue-table';
import VTable from '@/components/VTable/VTable.vue';
import VHeader from '@/components/VTable/VHeader.vue';
import VCell from '@/components/VTable/VCell.vue';
import Delete from './DeletePropertyType.vue';
...
const columns = [
    ...
    columnHelper.accessor('name', {
        header: ({ column }) => h(VHeader, { column, title: t('Property type name') }),
        cell: (info) => h(VCell, { cell: info, value: info.getValue() }),
    }),
            ...
            <div class="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl">
                <Card class="rounded-sm shadow-none">
                    ...
                    <CardFooter class="p-0">
                        <Pagination
                            :from="propertyTypes?.from"
                            :to="propertyTypes.to"
                            :total="propertyTypes.total"
                            :per_page="propertyTypes.per_page"
                            :links="propertyTypes?.links"
                            :first_page_url="propertyTypes?.first_page_url"
                            :last_page_url="propertyTypes?.last_page_url"
                            :next_page_url="propertyTypes.next_page_url"
                            :prev_page_url="propertyTypes?.prev_page_url"
                            class="p-4"
                        />
                    </CardFooter>
                </Card>
