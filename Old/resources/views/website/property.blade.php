    ...
    <section class="relative bg-gray-950 pt-14 px-0  ">
        <div class="splide default " data-splide='{"type":"loop","autoplay":true,"pagination":false}'>
            <div class="splide__track w-full">
                ...
                @foreach ($property->getMedia('posts') as $banner)
    ...
    <section class="!max-h-16">
        ...
        <div class="splide__track !h-16 w-full">
                <ul class="splide__list  justify-center items-center">
                    @foreach ($property->getMedia('posts') as $thumb)
    ...
    <section class="py-8 px-2 sm:px-6 lg:px-24 bg-gray-200 grid grid-cols-12 gap-8 text-gray-700">
        <div class=" col-span-12 md:col-span-8 relative h-fit " >
            ...
            <div class="bg-white rounded-sm p-4 px-8 mb-2 w-full" >
                <h1 class="text-2xl font-semibold py-2 first-letter:uppercase lowercase">
                    {{ $property->propertyFor->slug_text . ' ' . $property->titulo }}</h1>
                ...
                <div class="mt-2 flex ">
                    &nbsp; <strong class="text-2xl font-bold text-green-400">{{ $property->price }}</strong>
                </div>
                <div>
                    <span
                    ...
                    dark:text-yellow-300 uppercase">
                        {{ $property->regraDeBusinessRule->name ?? '' }}
                    </span>

                    <span
                        class="bg-yellow-100 text-yellow-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded-sm dark:bg-yellow-900
                     dark:text-yellow-300 uppercase">
                        {{ $property->intermediationRule->code ?? '' }}
                    </span> &nbsp;
            ...
            <div class="bg-white rounded-sm p-4 px-8 mb-2 w-full grid grid-cols-2 gap-2" >
              <div class="col-span-2">
                ...
                @if ($property->ano)
                    <div class="text-sm">
                        Ano de construção : <strong>&nbsp; {{ $property->ano }}</strong>
                    </div>
                @endif

                @if ($property->area)
                    <div class="text-sm">
                        Área (m²) : <strong>&nbsp; {{ $property->area }}</strong>
                    </div>
                @endif

                @if ($property->andares)
                    <div class="text-sm">
                        Andares : <strong>&nbsp; {{ $property->andares }}</strong>
                    </div>
                @endif

                @if ($property->quartos)
                    <div class="text-sm">
                        Quartos : <strong>&nbsp; {{ $property->quartos }}</strong>
                    </div>
                @endif

                @if ($property->suites)
                    <div class="text-sm">
                        Suítes : <strong>&nbsp; {{ $property->suites }}</strong>
                    </div>
                @endif

                @if ($property->banheiros)
                    <div class="text-sm">
                        Banheiros : <strong>&nbsp; {{ $property->banheiros }}</strong>
                    </div>
                @endif

                @if ($property->piscinas)
                    <div class="text-sm">
                        Piscinas : <strong>&nbsp; {{ $property->piscinas }}</strong>
                    </div>
                @endif

                @if ($property->garagens)
                    <div class="text-sm">
                        Garagens : <strong>&nbsp; {{ $property->garagens }}</strong>
                    </div>
                @endif
                @if ($property->tipo_de_property)
                    <div class="text-sm">
                        Tipo: <strong>&nbsp; {{ $property->tipo_de_property->nome }}</strong>
                    </div>
                @endif

                @if ($property->condition)
                    <div class="text-sm">
                        Condição : <strong>&nbsp; {{ $property->condition->nome }}</strong>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-sm p-1 px-8 mb-2 w-full" >
                ...
                <div class="grid grid-cols-8 sm:grid-cols-10 justify-center justify-items-center gap-4 ">
                    @foreach ($property->getMedia('posts') as $image)
            ...
            <div class="bg-white rounded-sm p-4 px-8 mb-2 w-full" id="comments">
                ...
                <div>
                    ...
                    @endif
                    <form action="{{ route('post.property.comment', ['property' => $property->slug]) }}" method="post">
                        @csrf
                        <div class="mb-6">
                ...
                @if ($property->comentarios->isNotEmpty())
                    @foreach ($property->comentarios->sortByDesc('created_at') as $comment)
        ...
        <div class=" col-span-12 md:col-span-4 h-fit md:sticky md:top-20  ">
            ...
            <div class=" bg-white p-4 px-4 lg:p-8 lg:py-2 my-2 w-full  hidden md:grid" >
                ...
                <div class="py-2 grid grid-cols-1 lg:grid-cols-2">
                    <a href="tel:{{ $property->corretor->contacto }}"
            ...
            <div class="bg-white p-4 mt-2 lg:mt-0 px-4 lg:p-8  lg:py-2 w-full grid">
                ...
                {{ $property->corretor->avatar?->img()->attributes(['class' => 'object-cover w-full h-auto']) }}
           </div>
            @endif
        </div>
    </section>

    <!-- Start::related-Propertys -->
    <section   class="py-8 px-2 sm:px-6 lg:px-24 bg-gray-100 dark:bg-gray-700 font-['Open_Sans'] text-gray-700" itemscope
        ...
        <div class="splide" id="new-added-houses" data-splide='{"type":"loop","autoplay":true}'>
            ...
            <div class="splide__track">
                <!-- Slides -->
                <ul class="splide__list">
                    @foreach ($property->relectedPropertys() as $property)
                                                    ...
                                                    ($property->endereco ? ', ' . $property->endereco : '') }}
                                                                        ...
                                                                        <div class="font-semibold text-sm text-gray-600 grid grid-cols-2">
                                                                            <p class="bg-orange-100 p-1 px-2 rounded-sm mr-2 line-clamp-1">
                                                                                {{ $property->tipo_de_property->nome }}</p>
                                                                            <p class="bg-orange-100 p-1 px-2 rounded-sm mr-2 line-clamp-1 ">
                                                                                {{ $property->status->nome }}</p>
          ...
          <div class="relative w-full max-w-md max-h-full">
              ...
              <div class="relative transition duration-150 ease-in-out bg-white rounded-sm shadow dark:bg-gray-700">
                  ...
                  <div class="px-6 py-6 lg:px-8">
                    <form action="{{ route('post.property.message',[
                        'property' =>$property->slug
                    ])}}" method="post">
                        @csrf
                    <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-2 justify-items-stretch">
                        ...
                        <div class="mb-2">
                          ...
                          focus:ring-gray-500 border-gray-500 focus:border-gray-500" value="" placeholder="Exemplo@mproperty.com" />
                          @error('email')
                           <span class="text-red-500 text-sm font-semibold ">{{ $message }}</span>
                          @enderror
